...
import pt314.blocks.game.LoadBoard;
...
public class SimpleGUI extends JFrame implements ActionListener {
	...
	private void initMenus() {
		...
		newGameMenuItem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				else JOptionPane.showMessageDialog(SimpleGUI.this, "Coming soon...");
			}
		});
	...
	private void initBoard() {
		lBoard = new LoadBoard("/home/blondprodigy/blocks/app/res/puzzles/myPuzzle.txt");
		try{
			board = lBoard.load();
		...
		buttonGrid = new GridButton[board.getWidth()][board.getHeight()];
		rows = board.getWidth();
	...
	private void moveSelectedBlockTo(int row, int col) {
		...
		if (!board.moveBlock(selectedBlockRow, selectedBlockCol, dir, dist)) {
			...
			updateUI();
			if(selectedBlock instanceof TargetBlock){
				if(col == cols-1){
